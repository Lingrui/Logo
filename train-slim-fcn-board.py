...
flags.DEFINE_boolean('aaalgo', False, '')
flags.DEFINE_boolean('reg', True, '')
...
def fcn_loss (logits, labels):
    ...
    if FLAGS.reg:
        ...
        loss = tf.identity(xe, name='lo')
        return loss, [acc, xe, loss]
...
def create_picpac_stream (db_path, is_training):
    ...
    if is_training:
        augments = [
                  ...
                  #{"type": "augment.rotate", "min":-15, "max":15},
                  #{"type": "augment.scale", "min":0.9, "max":1.2},
                  #{"type": "augment.add", "range":10},
                ]
    else:
        augments = []
    ...
    config = {"db": db_path,
              ...
              "transforms": augments + [
                  ...
                  #{"type": "clip", "width": 352, "height": 640, "round": FLAGS.stride},
                  {"type": "clip", "min_height": 352, "max_height": 352, "min_width": 640, "max_width": 640, "round": FLAGS.stride},
                  {"type": "rasterize"},
                  ]
             }
...
def main (_):
    ...
    if FLAGS.aaalgo:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose, slim.max_pool2d],
            ...
            logits, FLAGS.stride = getattr(fcn_nets, FLAGS.backbone)(X-PIXEL_MEANS, num_classes=FLAGS.classes)
    ...
    with tf.Session(config=config) as sess:
        ...
        while epoch < FLAGS.max_epochs:
            ...
            for _ in progress:
                ...
                #progress.set_description(str(np.unique(labels))) #metrics_txt)
